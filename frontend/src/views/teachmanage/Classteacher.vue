  ...
  <div id="datagrid">
    ...
    <el-pagination
      ...
      :total="total">
        ...
        <!-- 编辑列表 -->
  <el-dialog title="班主任信息" center :visible.sync="editDialogFormVisible" :close-on-click-modal="false">
      <el-form :model="form" label-width="100px">
          <el-form-item label="姓名">
            <el-select v-model="form.teacher_id" clearable filterable placeholder="请查找或选择">
                  <el-option v-for="item in teachers" :key="item.id" :label="item.name" :value="item.id">
                  </el-option>
            </el-select>
          </el-form-item>
        <el-form-item label="班级"  prop="class_id">
           <el-input-number v-model="form.class_id" :min="1" :max="50" label="请填写班级"></el-input-number>
        </el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      pageSize: 10,
      session_id: null,
      templateFile: config.site + '/xls/class_teacher.xls',
      downloadFile: config.site + '/xls/班主任管理.xls',
      multiSelect: []
    };
  },
  methods: {
